<template>
  <div class="mt-10">
    <div class="flex flex-wrap justify-center">
      <!-- List of Water Sessions -->
      <section>
        <div>
          <div class="flex justify-between mb-2">
            <h1 class="text-3xl text-indigo-700">Water Sessions</h1>
...
export default {
  setup() {
    ...
    const reloadData = async () => {
      state.sessions = await WaterSessionService.getAll();
    };
    ...
    const deletePressed = async (id) => {
      await WaterSessionService.deleteOne(id);
      state.wellness = await WaterSessionService.getAll();
    };
